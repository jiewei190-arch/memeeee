...
<div class="filters" id="filters"></div><section class="panel"><table><thead><tr><th>Time</th><th>Chain</th><th>Coin</th><th>Score</th><th>Status</th><th>Liquidity</th><th>1h volume</th><th>Reason</th></tr></thead><tbody id="rows"></tbody></table></section>
</main><script>
...
const chains=['all',...new Set(r.map(x=>x.chain))];document.querySelector('#filters').innerHTML=chains.map(c=>`<button class="${c===selected?'on':''}" onclick="selected='${c}';refresh()">${c.toUpperCase()}</button>`).join('');
document.querySelector('#rows').innerHTML=r.filter(x=>selected==='all'||x.chain===selected).map(x=>`<tr><td>${new Date(x.observed_at).toLocaleTimeString()}</td><td>${x.chain}</td><td class="mono">${x.symbol}<br><small>${x.address.slice(0,8)}…</small></td><td>${x.score}</td><td class="${x.status}">${x.status}</td><td>${money(x.liquidity_usd)}</td><td>${money(x.volume_h1_usd)}</td><td>${JSON.parse(x.reasons_json||'[]')[0]||'passed gates'}</td></tr>`).join('');}
